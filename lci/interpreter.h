...
#define getColor(value) (value->data.color)
#define getTexture(value) (value->data.texture)
#define getTexture2D(value) getTexture(value)
#define getVector2(value) (value->data.vector2)
...
typedef union {
  ...
  Color color;  /**< Color, 4 components, R8G8B8A8 (32bit) */
  Texture2D texture;  /**< Texture, tex data stored in GPU memory (VRAM) */
  Vector2 vector2;
} ValueData;
...
ValueObject *createTexture2DValueObject(Texture2D);
ValueObject *createColorValueObject(Color);
ValueObject *createVector2ValueObject(Vector2);
